cdef AlwaysTrue _raise_unclosed(const char *what, Py_ssize_t start) nogil except True:
    with gil:
        raise Json5EOF(f'Unclosed {what} starting near {start}')


cdef AlwaysTrue _raise_stray_character(const char *what, Py_ssize_t where) nogil except True:
    with gil:
        raise Json5IllegalCharacter(f'Stray {what} near {where}')


cdef AlwaysTrue _raise_expected_sc(const char *char_a, uint32_t char_b, Py_ssize_t near, uint32_t found) nogil except True:
    with gil:
        raise Json5IllegalCharacter(f'Expected {char_a} or U+{char_b:04x} near {near}, found U+{found:04x}')


cdef AlwaysTrue _raise_expected_s(const char *char_a, Py_ssize_t near, uint32_t found) nogil except True:
    with gil:
        raise Json5IllegalCharacter(f'Expected {char_a} near {near}, found U+{found:04x}')


cdef AlwaysTrue _raise_expected_c(uint32_t char_a, Py_ssize_t near, uint32_t found) nogil except True:
    with gil:
